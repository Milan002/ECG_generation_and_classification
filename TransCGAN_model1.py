...
class Generator(nn.Module):
    def _init_(self, seq_len=150, channels=3, num_classes=9, latent_dim=100, data_embed_dim=10, 
                label_embed_dim=10 ,depth=3, num_heads=5, 
                forward_drop_rate=0.5, attn_drop_rate=0.5, lstm_hidden_dim=64, lstm_layers=2):
        ...
        self.bilstm = nn.LSTM(
            input_size=self.data_embed_dim,
        ...
        self.deconv = nn.Sequential(
            nn.Conv2d(self.lstm_hidden_dim * 2, self.channels, 1, 1, 0)
        )
    ...
    def forward(self, z, labels):
        ...
        bilstm_output = bilstm_output.unsqueeze(1)  # Add channel dimension
        
        output = self.deconv(bilstm_output.permute(0, 3, 1, 2))  # Permute for Conv2D
        return output 
...
class FeedForwardBlock(nn.Sequential):
    def _init_(self, emb_size, expansion, drop_p):
        super().__init__(
...
class ClassificationHead(nn.Sequential):
    def _init_(self, emb_size=100, adv_classes=2, cls_classes=10):
        super().__init__()
...
class PatchEmbedding_Linear(nn.Module):
    def _init_(self, in_channels = 21, patch_size = 16, emb_size = 100, seq_length = 1024):
        super().__init__()
